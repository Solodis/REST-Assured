...
public class DashboardTest extends BaseTest {
	...
	public void addDashboard() {
		...
		dashboardID = given().header("Authorization", auth).body(dash).when().contentType("application/json")
		        .post("/dashboard").then()
		        /**
		         * Checking that time of execution is less then 700 mls
		         */
		        .time(lessThan(700l)).assertThat().statusCode(201).extract().response().path("id");
	...
	public void receivingOfDashboards() {
		...
		given().header("Authorization", auth).when().contentType(ContentType.JSON).get("/dashboard").then().assertThat()
	...
	public void testPutRequest() {
		...
		given().header("Authorization", auth).body(dash).when().contentType(ContentType.JSON)
		        .put("/dashboard/{dashboardID}", dashboardID).then().log().all().assertThat().statusCode(200);
	...
	public void checkDashboardFields() {
		...
		Response res = given().header("Authorization", auth).when().contentType(ContentType.JSON).get("/dashboard")
		        .andReturn();
	...
	public void testDeleteRequest() {
		...
		Response res = given().headers("Authorization", auth).when().contentType(ContentType.JSON)
		        .delete("/dashboard/{DashboardID}", dashboardID).then().assertThat().statusCode(200).extract()
		        .response();
