...
public class LaunchTest extends BaseTest {
	...
	public void allLaunchs(String value) {
		...
		Response res = given()
		        ...
		        .body("content[0].owner", equalTo("vyacheslav_utenkov")).assertThat().statusCode(200).extract()
		        .response();
	...
	public void allLaunchsSecondTest() {
		...
		Response res = given().header("Authorization", auth).when().get("/launch").then().contentType(ContentType.JSON)
		        .body("content[0].owner", equalTo("vyacheslav_utenkov")).assertThat().statusCode(200).extract()
		        .response();
		int i = 0;
		while (res.path("content[" + i + "].statistics.executions.total") != null) {
	...
	public static Object[][] provideStatus() {
		...
		return new Object[][] { new Object[] { status.getString("Status.FAILED") },
